...
namespace DarkKapoRR.Validaciones
{
    public class CrearJugadorDTOValidador : AbstractValidator<CrearJugadorDTO>
    {
        ...
        {
            RuleFor(n => n.Nombre).NotEmpty().WithMessage("{PropertyName} es requerido")
                                  .Length(3, 50).WithMessage("{PropertyName} tiene {TotalLength} letras. Debe tener una longitud entre {MinLength} y {MaxLength} letras.");
